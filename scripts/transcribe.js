...
const chunkFiles = async (filePath, chunkDuration, dirPath) => {
  ...
  for (let start = 0; start < duration; start += chunkDuration) {
    const chunkFilePath = path.join(dirPath, `chunk_${Date.now()}_${start}.m4a`);
    chunkFilePaths.push(chunkFilePath);
    await splitChunk(filePath, chunkFilePath, start, Math.min(chunkDuration, duration - start));
  }
...
const sendFileToOpenAI = async (filePath, previous_transcript, retries = 3, backoff = 300) => {
  try {
    ...
    if (retries > 0 && error.code === 'ECONNRESET') {
      console.log(`Retrying... ${retries} retries left. Error: ${error.message}`);
      await new Promise(resolve => setTimeout(resolve, backoff));
      return sendFileToOpenAI(filePath, previous_transcript, retries - 1, backoff * 2);
    } else {
...
export const transcribeWithOpenAI = async (filePath) => {
  ...
  for (let i = 0; i < chunkFilePaths.length; i++) {
    ...
    const transcription = await sendFileToOpenAI(chunkFilePaths[i], transcriptions[i]);
